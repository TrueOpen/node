...
// simulationAppStateFn keeps independently randomized SDK module genesis
// parameters inside the application's cross-module validity domain. Staking
// prunes the snapshot at H during BeginBlock H+historical_entries, while the
// freeze vote deadline is inclusive, so the simulator needs one extra entry.
func simulationAppStateFn(app *App) simulationtypes.AppStateFn {
	return simtestutil.AppStateFnWithExtendedCb(
		app.AppCodec(), app.SimulationManager(), app.DefaultGenesis(),
		func(rawState map[string]json.RawMessage) {
			var hubGenesis hubtypes.GenesisState
			if err := app.AppCodec().UnmarshalJSON(rawState[hubtypes.ModuleName], &hubGenesis); err != nil {
				panic(err)
			}
			var stakingGenesis stakingtypes.GenesisState
			if err := app.AppCodec().UnmarshalJSON(rawState[stakingtypes.ModuleName], &stakingGenesis); err != nil {
				panic(err)
			}
			voteWindow := hubGenesis.Params.Freeze.FreezeSignalVoteWindowBlocks
			if voteWindow >= uint64(^uint32(0)) {
				panic("freeze vote window cannot fit staking historical_entries")
			}
			minimumEntries := uint32(voteWindow + 1)
			if stakingGenesis.Params.HistoricalEntries < minimumEntries {
				stakingGenesis.Params.HistoricalEntries = minimumEntries
				rawState[stakingtypes.ModuleName] = app.AppCodec().MustMarshalJSON(&stakingGenesis)
			}
		},
	)
}
