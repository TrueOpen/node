...
func TestRegisteredTaskRPCsHaveConcreteKeeperMethods(t *testing.T) {
	_, thisFile, _, ok := runtime.Caller(0)
	require.True(t, ok)
	methods := taskKeeperReceiverMethods(t, filepath.Dir(thisFile))
	assertConcreteTaskServiceMethods(t, types.Msg_serviceDesc, methods["msgServer"])
	assertConcreteTaskServiceMethods(t, types.Query_serviceDesc, methods["queryServer"])
}

func assertConcreteTaskServiceMethods(t *testing.T, descriptor grpc.ServiceDesc, implementation map[string]struct{}) {
	t.Helper()
	missing := []string{}
...
func taskKeeperReceiverMethods(t *testing.T, directory string) map[string]map[string]struct{} {
	t.Helper()
