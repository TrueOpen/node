...
func TestServiceBondEffectiveIndexActivatesFreshCandidateSlot(t *testing.T) {
	...
	bond, err := f.keeper.ServiceBond.Get(registerCtx, types.NewServiceBondKey(operator.Address))
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.True(t, has)
	reverse, err := f.keeper.OperatorCandidateSlot.Get(registerCtx, operator.Address)
	require.NoError(t, err)
	current, err := f.keeper.CandidateSlotCurrent.Get(registerCtx, reverse.Slot)
	require.NoError(t, err)
	...
	bond, err = f.keeper.ServiceBond.Get(activationCtx, types.NewServiceBondKey(operator.Address))
	require.NoError(t, err)
	...
	reverse, err = f.keeper.OperatorCandidateSlot.Get(activationCtx, operator.Address)
	require.NoError(t, err)
