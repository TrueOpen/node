...
func TestActivateDueVrfKeysRetiresTheIteratedRow(t *testing.T) {
	...
	t.Run("state disagrees with the row", func(t *testing.T) {
		...
		require.NoError(t, f.keeper.VrfKey.Set(f.ctx, disagreeing, types.VrfKeyState{
		...
		state, err := f.keeper.VrfKey.Get(f.ctx, disagreeing)
		require.NoError(t, err)
	...
	t.Run("overdue rotation is promoted and the row retired", func(t *testing.T) {
		...
		require.NoError(t, f.keeper.VrfKey.Set(f.ctx, rotating, types.VrfKeyState{
		...
		history, err := f.keeper.VrfKeyHistory.Get(f.ctx, types.NewVrfKeyHistoryKey(rotating, 1))
		require.NoError(t, err)
...
func TestActivateDueVrfKeysLeavesFutureRowsAlone(t *testing.T) {
	...
	require.NoError(t, f.keeper.VrfKey.Set(f.ctx, operator, types.VrfKeyState{
